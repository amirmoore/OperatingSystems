...
int main() {
  ...
  list_add_at_index(mylist, 888, 4);
  if(strcmp("90->70->60->888->50->40->20->10->5->NULL",listToString(mylist)) != 0)
  {
    printf("list_Add_at_index(mid): FAILED \n");
  }
  list_print(mylist);

  if (list_remove_from_front(mylist) != 90) {
    printf("list_remove_to_front: FAILED\n");
  }

  if (list_remove_from_back(mylist) != 5) {
    printf("list_remove_from_back: FAILED\n");
  }
  if(strcmp("70->60->888->50->40->20->10->NULL", listToString(mylist)) != 0)
  {
    printf("post pops state : FAILED\n");
  }
  list_print(mylist);
